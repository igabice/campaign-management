...
export const updateSocialMediaStats = async (): Promise<void> => {
  try {
    ...
    for (const account of socialMediaAccounts) {
      try {
        logger.info(`Updating stats for ${account.platform} account ${account.id}`);
        ...
        logger.info(`Updated stats for ${account.platform} account ${account.id}`);
      } catch (error) {
        logger.error(`Failed to update stats for account ${account.id}:`, error);
      }
    }

    logger.info(`Updated stats for ${socialMediaAccounts.length} social media accounts`);
  } catch (error) {
