...
const GenerateContentPlanInputSchema = z.object({
  ...
  description: z
    .string()
    .describe("A detailed description of the content to be generated."),
...
const PostSchema = z.object({
  ...
  dayOffset: z
    .number()
    .describe("The number of days from today to schedule the post."),
  autoPublish: z
    .boolean()
    .describe("Whether the post should be auto-published."),
  status: z
    .enum(["Draft", "Scheduled", "Published", "Needs Verification"])
    .describe("The status of the post."),
});

const GenerateContentPlanOutputSchema = z.object({
  posts: z
    .array(PostSchema)
    .describe("A list of generated posts for the content plan."),
});
...
const generateContentPlanPrompt = ai.definePrompt({
  ...
  - The tweet content (keep under 280 characters).
...
const generateContentPlanFlow = ai.defineFlow(
  ...
  async (input: GenerateContentPlanInput) => {
    const { output } = await withTimeout(
      generateContentPlanPrompt(input),
      60000
    ); // 60 second timeout
